...
configured in country channel lists. Therefore, you must choose your country list in the `Country ` combo box. The
default country list is `Germany`. If your Zattoo account is registered in Switzerland or Austria you should select the
appropriated list.  
...
entries. The Germany and Switzerland list did not change since 1.0.2.0 and therefore, some channels are missing, or even
names have changed.  
The Austria list, at the time, has only valid entries but not all 96 channels which are provided by Zattoo for Austria
were included. Currently, the following channels are configured:
...
You need to use your own channel list. Please go to your plugin settings in TV-Browser and follow the instructions in
the `Create your own channel list` and `Adopt your own channel list` area in the TV-Browser settings panel.

### My country is not available as a predefined channel list

Same procedure as if only some channels are missing. See
[Some programs are not available for Zattoo](#some-programs-are-not-available-for-zattoo).

And yes, you must select a channel list as `source`, even if you expect that it will not fit at all. As a result, all
